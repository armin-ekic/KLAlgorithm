...
def main():
    ...
    lowerEdgeMatrix = edgeMatrix(nodes,modE)
    ...
    for i in range(4): #here we are doing 4 total iterations
        ...
        for x in range(0, len(nodes), 2): #here we will loop through half the number of nodes (lock 2 per iteration)
            g = [-999]*len(nodes)  # matrix used to store gains of edges, initialized to row of min values

            g = gain(partA, D, nodes, lowerEdgeMatrix, g)

            print(g)

            maxKey, maxList = findMaxGain(g, partA, partB, nodes) #this will find the key (tuple representing edge) pertaining to the max gain value

            print("before node swap")
            swapNodes(nodes, maxKey, partA, partB, D, lowerEdgeMatrix)

            gainValues.append(maxList[maxKey] + gainValues[-1]) #append the max calculated gain to the accumulated gain, after adding it to the most recent value
